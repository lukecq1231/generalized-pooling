...
# feedforward layer: affine transformation + point-wise nonlinearity
def param_init_multihead_attention(options, params,
                                   num_heads=3,
                                   prefix='multihead_attention', nin=None, nout=None, **kwargs):
    if nin is None:
        nin = options['dim']
    if nout is None:
        nout = options['dim']
    params[_p(prefix, 'W1')] = norm_weight(nin, nout*num_heads, scale=0.01, ortho=False)
    params[_p(prefix, 'W2')] = norm_weight(nout, nin*num_heads, scale=0.01, ortho=False)
    params[_p(prefix, 'b1')] = numpy.zeros((nout*num_heads,)).astype('float32')
    params[_p(prefix, 'b2')] = numpy.zeros((nin*num_heads,)).astype('float32')

    return params

def multihead_attention(tparams, options, 
                        inputs, mask, num_heads=3,
                        prefix='multihead_attention', **kwargs):
    # inputs (step1, batch, dim)
    step = inputs.shape[0]
    batch = inputs.shape[1]
    nout = tparams[_p(prefix, 'W1')].shape[1]//num_heads
    nin = tparams[_p(prefix, 'W1')].shape[0]

    multiatt = relu(tensor.dot(inputs, tparams[_p(prefix, 'W1')]) + tparams[_p(prefix, 'b1')]) #(step, batch, num_heads*nout)
    multiatt = multiatt.reshape([step, batch, num_heads, nout]) #(step, batch, num_heads, nout)
    multiatt = multiatt.dimshuffle(2,0,1,3) #(num_heads, step, batch, dim)
    multiatt = tensor.batched_tensordot(multiatt, tparams[_p(prefix, 'W2')].reshape([nout, num_heads, nin]).dimshuffle(1,0,2), axes=[3,1]) #(num_heads, step, batch, nin)
    multiatt += tparams[_p(prefix, 'b2')].reshape([num_heads, nin])[:,None,None,:] #(num_heads, step, batch, nin)
    multiatt = multiatt.dimshuffle(1,2,3,0) #(step, batch, nin, num_heads)

    multiatt = tensor.exp(multiatt - multiatt.max(0, keepdims=True)) #(step, batch, nin, num_heads)
    multiatt = multiatt * mask[:,:,None,None] #(step, batch, nin, num_heads)
    multiatt = multiatt / (multiatt.sum(0, keepdims=True)  + 1e-8)

    outputs = (inputs.dimshuffle(0,1,2,'x') * multiatt).sum(0) #(batch, nin, num_heads)

    return outputs, multiatt
...
def init_params(options, worddicts):
    ...
    params = param_init_multihead_attention(options, params,
                                            num_heads=options['num_heads'],
                                            prefix='multihead_attention', 
                                            nin=options['dim']*2, 
                                            nout=options['dim'])

    # classifier
    params = get_layer('ff')[0](options, params, prefix='ff_layer_1',
                                nin=options['dim']*2*4*options['num_heads'], nout=options['dim'], ortho=False)
    params = get_layer('ff')[0](options, params, prefix='ff_layer_2',
                                nin=options['dim'] + options['dim']*2*4*options['num_heads'], nout=options['dim'], ortho=False)
...
def build_model(tparams, options):
    ...
    att_1, multiatt_1 = multihead_attention(tparams, options, ctx1, x1_mask, num_heads=options['num_heads'])
    att_2, multiatt_2 = multihead_attention(tparams, options, ctx2, x2_mask, num_heads=options['num_heads'])

    rep1_origin = att_1 #(batch, nin, num_heads)
    rep2_origin = att_2 #(batch, nin, num_heads)

    rep1 = att_1.reshape([att_1.shape[0], att_1.shape[1]*att_1.shape[2]]) #(batch, num_heads*nin)
    rep2 = att_2.reshape([att_2.shape[0], att_2.shape[1]*att_2.shape[2]]) #(batch, num_heads*nin)
    ...
    opt_ret['rep1'] = rep1
    opt_ret['rep2'] = rep2
    opt_ret['rep1_origin'] = rep1_origin
    opt_ret['rep2_origin'] = rep2_origin
    opt_ret['multiatt_1'] = multiatt_1    #(step, batch, nin, num_heads)
    opt_ret['multiatt_2'] = multiatt_2    #(step, batch, nin, num_heads)
...
def split(x, num, axis=1):
    out = []
    dim = x.shape[axis]/num
    if axis == 1:
        for n in range(num):
            out.append(x[:, n*dim:(n+1)*dim])
...
def train(
    ...
    # apply L2 regularization on W1
    if decay_w1 > 0.:
        decay_w1 = theano.shared(numpy.float32(decay_w1), name='decay_w1')
        weight_decay = 0.
        W1_list = split(tparams[_p('multihead_attention', 'W1')], model_options['num_heads'])

        for i in range(0,model_options['num_heads']):
            for j in range(i+1,model_options['num_heads']):
                weight_decay += relu(1-((W1_list[i] - W1_list[j]) ** 2).sum())
        weight_decay *= decay_w1
        cost += weight_decay

    if decay_rep > 0.:
        decay_rep = theano.shared(numpy.float32(decay_rep), name='decay_rep')
        weight_decay = 0.

        for i in range(0,model_options['num_heads']):
            for j in range(i+1,model_options['num_heads']):
                weight_decay += relu(1-((opt_ret['rep1_origin'][:,:,i] - opt_ret['rep1_origin'][:,:,j]) ** 2).sum(1).mean())
                weight_decay += relu(1-((opt_ret['rep2_origin'][:,:,i] - opt_ret['rep2_origin'][:,:,j]) ** 2).sum(1).mean())

        weight_decay *= decay_rep
        cost += weight_decay

    if decay_att > 0.:
        decay_att = theano.shared(numpy.float32(decay_att), name='decay_att')
        weight_decay = 0.

        for i in range(0,model_options['num_heads']):
            for j in range(i+1,model_options['num_heads']):
                weight_decay += relu(1-((opt_ret['multiatt_1'][:,:,:,i] - opt_ret['multiatt_1'][:,:,:,j]) ** 2).sum([0,2]).mean())
                weight_decay += relu(1-((opt_ret['multiatt_2'][:,:,:,i] - opt_ret['multiatt_2'][:,:,:,j]) ** 2).sum([0,2]).mean())

        weight_decay *= decay_att
        cost += weight_decay
